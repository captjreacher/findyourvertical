...
create table if not exists public.fyv_event_emit_failures (
  ...
  attempted_entity_type text,
  attempted_entity_id uuid,
  attempted_source_system text,
  attempted_status text,
  attempted_entity_ref text,
  attempted_correlation_id text,
...
revoke all on public.fyv_event_emit_failures from public;
grant select, insert on public.fyv_event_emit_failures to service_role;
...
begin
  insert into public.fyv_event_emit_failures (
    ...
    attempted_entity_id,
    attempted_source_system,
    attempted_status,
    attempted_entity_ref,
    attempted_correlation_id,
    attempted_payload,
    error_sqlstate,
    error_message,
    error_detail,
    ...
    p_error_sqlstate,
    coalesce(p_error_message, 'unknown event emission failure'),
    p_error_detail,
    p_error_hint,
    p_error_context
  );
exception when others then
  raise warning 'FYV event emit diagnostic write failed in % for event %: [%] %',
    p_wrapper_name,
    p_event_type,
    sqlstate,
    sqlerrm;
...
revoke all on function public.fyv_record_event_emit_failure(
  text, text, text, uuid, jsonb, text, text, text, text, text, text, text, text, text
) from public;
