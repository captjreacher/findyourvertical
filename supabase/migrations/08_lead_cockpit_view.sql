...
--
-- Guarded so it is safe to apply when public.events does not exist yet
-- (e.g. a fresh local `supabase start`, where public.events comes from an
-- external baseline rather than an earlier migration, and this migration sorts
-- first). When public.events is absent, an empty-compatible view with identical
-- columns/types is created so the schema contract is preserved and boot does
-- not fail. When public.events exists, behaviour is unchanged.

BEGIN;

DO $$
DECLARE
  v_has_events boolean := to_regclass('public.events') IS NOT NULL;
  v_has_paperclip_issues boolean;
BEGIN
  IF v_has_events THEN
    -- ── Current behaviour (unchanged): real view over public.events ──────────
    ...
    -- Index over public.events (only meaningful when the table exists).
    EXECUTE $idx$
    ...
    $idx$;
  ELSE
    -- ── public.events absent (fresh bootstrap): empty-compatible view ────────
    -- Identical column names + types to the real view above (event_id uuid,
    -- lead_id text per public.events, metadata jsonb, etc.), returning no rows
    -- and with no dependency on public.events.
    EXECUTE $empty$
      CREATE OR REPLACE VIEW public.lead_cockpit_view AS
      SELECT
...
-- Create index for performance (guarded above; view always exists here).
